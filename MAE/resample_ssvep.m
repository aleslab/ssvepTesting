...
% first remove the pre-stimulus
numEpoch = round((trialLengthSecs-(preStimDuration+preStimDuration/2)) / epochLengthSecs); 
for iTrial = 1:length(datain.trial)
    ...
    for iCycle = 1:numEpoch
        begsample = round((preStimDuration*newFs)+epochLengthSecs*newFs*(iCycle-1)) + trialLengthSecs*newFs*(iTrial-1);
