function trl = lock2ssvep_LongRange_V2(cfg)
...
%                 endsample = begsample+meanSampPerCycle*numCyclesPerEpoch-1;
                endsample     = stimCyclesPlusOne(iCycle+numCyclesPerEpoch)-1; %One sample before the next cycle.
                offset        = 0;
                cycleLengthSamp = ceil((endsample - begsample) / numCyclesPerEpoch); % have to add 1 because the cycle is with +1 (since remove the one above? - still confused)
