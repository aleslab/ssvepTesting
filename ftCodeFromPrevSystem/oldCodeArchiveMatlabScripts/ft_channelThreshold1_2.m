function [cfg, cleanData, chanSubs, interpData] = ft_channelThreshold1_2(cfg, data)
...
for trlOp = 1: numTrials
	...
	for nCh = 1:128
		...
		%tmpArray(nCh,trlOp) = ~isempty(abs(cleanData.trial{trlOp}(nCh,:)) > 1600);
    end
...
for trlOp = 1: numTrials    
    tmpcfg = cfg;
	tmpcfg.trials = trlOp;
    tmpcfg.badchannel = chanSubs{trlOp};
