function [dat respTime condInfo] = eegResponseTime(dataDir,optns,condition)
...
for iCnd=condList,
    ...
    for iTrial = 1:length(condData),
        ...
        if isfield(optns,'lowpass') && ~isempty(optns.lowpass) && optns.lowpass==true;
...
%Justin's Epoch code
        if iTrial == 1
            ...
            nTime = size(thisTrialEEG,1)/nEpochs;
			%dat{iCnd} = zeros(nTime,allTrials,nEeg);
            %fullData = zeros(size(condData{iTrial}.EEG,1)*allTrials,nTotalEpochs,nEeg);
		...
		%thisTrialByEpoch = reshape(thisTrialEEG,[],iTrial,nEeg);
		dat{condIdx}(:,iTrial,:) = thisTrialEEG;
% 		for iChan = 1:nEeg,
% 			for iTime = 1:size(thisTrialEEG,1),
% 				dat{iCnd}(iTime,iTrial,iChan)= thisTrialEEG(iTime,iChan);
% 			end
% 		end
		%dat{iCnd}(:,iTrial,:)= thisTrialEEG;
