...
% static and motion at different duty-cycles (designMatrix*betas) 
% and plot the results of the fitted signal + residuals 
...
% for this, need the same time window across conditions
% so will repeat the cycle for the shorter window so that they are all 192
% timepoints (= 375ms)
...
title('regression on static conditions')
xlabel('time for stacked conditions');ylabel('Oz amplitude');
...
colormap('hot');
subplot(2,3,4); plotTopo(rms(betaWeights(:,1,:),3),cfg.layout); title('rms(onset)'); colorbar; %caxis([0 1.2]);
subplot(2,3,5); plotTopo(rms(betaWeights(:,2,:),3),cfg.layout); title('rms(offset)'); colorbar; %caxis([0 1.2]);
subplot(2,3,6); plotTopo(rms(residual(:,:),2),cfg.layout); title('rms(residual)'); colorbar; %caxis([0 1.2]);

saveas(gcf,['figures' filesep 'extractOnOffallStaticNewRef'],'png')


%% compute FFT for static and motion which will be use (cos and sin) to fit the area model
% reminder:nT is the number of timepoints for the timewindow of the lowest 
% freq - which is now the same across conditions (cycle has been repeated)
nbCond = length(fitData)/nT; 
fsample = avData.fsample;
...
amp = dataFFT; cos=dataFFT; sin=dataFFT;

for cond = 1:nbCond
    for ch=1:nCh
        clear data dataFourier dataRes dftData; 
        data = fitData(ch,nT*(cond-1)+1:nT*cond);
        waveForm(ch,:,cond,1) = data;

%         % fourier transform
%         dftData = data*dft;
%         %Select just the unique frequencies (kick out the symmetry)
%         dftData = dftData(1:nfr);
%         % Double the amplitude of the freqs that are represented twice
%         freqsToDouble = 2:(nfr-1+mod(nT,2));
%         dftData(:,freqsToDouble) = 2*dftData(:,freqsToDouble);
%         % Now normalize by the number of points in the fourier transform to
...
%         sin = -imag(dftData);
        ...
        amp(ch,:,cond,1) = abs(dataFFT(ch,:,cond,1));
        cos(ch,:,cond,1) = real(dataFFT(ch,:,cond,1));
        sin(ch,:,cond,1) = -imag(dataFFT(ch,:,cond,1));
        ...
        amp(ch,:,cond,2) = abs(dataFFT(ch,:,cond,2));
        cos(ch,:,cond,2) = real(dataFFT(ch,:,cond,2));
        sin(ch,:,cond,2) = -imag(dataFFT(ch,:,cond,2));
    end
end
...
poolSignal = []; poolRes=[];
idxHarm = {5:4:19; 5:4:19;5:4:19;5:4:19;5:4:19;...
    3:2:19 ;3:2:19 ;3:2:19 ;3:2:19 ;3:2:19 ; ...
    2:19;2:19;2:19;2:19;2:19}; % keep only the harmonics up to 50 Hz
for cond = 1:size(cos,3)
    poolSignal = [poolSignal cos(:,idxHarm{cond},cond,1) sin(:,idxHarm{cond},cond,1)]; 
    poolRes = [poolRes cos(:,idxHarm{cond},cond,2) sin(:,idxHarm{cond},cond,2)]; 
end
[beta, betaUn,lambda] = fitEEGTemplates(poolSignal,biosemi128,1);
[betaRes, betaUnRes,lambdaRes] = fitEEGTemplates(poolRes,biosemi128,1);
[betaAll, betaUnAll,lambdaAll] = fitEEGTemplates([poolSignal poolRes],biosemi128,1);

% compute power 
bounds = cumsum([0 8 8 8 8 8 18 18 18 18 18 36 36 36 36 36 8 8 8 8 8 18 18 18 18 18 36 36 36 36 36]);
for cond=1:30
    power(:,cond) = sum ( betaAll(:,bounds(cond)+1:2:bounds(cond+1)).^2 + betaAll(:,bounds(cond)+2:2:bounds(cond+1)).^2 ,2);
end
clear amp
amp = sqrt(power);
col={'b','r','g'};
...
for roi = 1:18
    ...
    plot(dcVal,amp(roi,1:5),['.-'  col{1}],'MarkerSize',40,'LineWidth',2)
    plot(dcVal,amp(roi,6:10),['.-'  col{2}],'MarkerSize',40,'LineWidth',2)
    plot(dcVal,amp(roi,11:15),['.-'  col{3}],'MarkerSize',40,'LineWidth',2)
    xticks([0:12.5:100]);
    title(biosemi128.listROIs(roi))
%     ylim([min(amp(:)) max(amp(:))]);
    ylim([min(min(amp(:,1:15))) max(max(amp(:,1:15)))]);
    ylabel('amplitude (a.u.)')
end
xlabel('Duty Cycle')
legend('10','5','2.5','10mov','5mov','2.5mov','Location','Best')
saveas(gcf,['figures' filesep 'extractSignal.png'])


figure;set(gcf, 'Position', [0 0 1600 700])
for roi = 1:18
    subplot(3,6,roi); hold on;
    plot(dcVal,amp(roi,16:20),['.-'  col{1}],'MarkerSize',40,'LineWidth',2)
    plot(dcVal,amp(roi,21:25),['.-'  col{2}],'MarkerSize',40,'LineWidth',2)
    plot(dcVal,amp(roi,26:30),['.-'  col{3}],'MarkerSize',40,'LineWidth',2)
    xticks([0:12.5:100]);
    title(biosemi128.listROIs(roi))
%     ylim([min(amp(:)) max(amp(:))]);
    ylim([min(min(amp(:,16:30))) max(max(amp(:,16:30)))]);
    ylabel('amplitude (a.u.)')
end
xlabel('Duty Cycle')
legend('10','5','2.5','10mov','5mov','2.5mov','Location','Best')
saveas(gcf,['figures' filesep 'extractResiduals.png'])
