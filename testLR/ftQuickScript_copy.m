...
ft_defaults
cfg.dataset = '/Users/marleneponcet/Documents/ssvep/test/testSSVEP/SSVEPtest3.bdf';
...
cfg.trialfun = 'lock2SsvepTag_copy'; 
...
spectrumDatAll = spectrumDat*2/length(fftDat); % estimate of the power amplitude = 2*A/N because N sample data amplitude A results in a DFT peak of AN/2
freqs = [1:length(spectrumDatAll)] * 1/(timelock.time(end)-timelock.time(1));
figure; hold on; plot(freqs(freqs<80),spectrumDatAll(freqs<80),'r');
pdSpecPlot_copy(freqs(1:80),spectrumDatAll(1:80),[]);
